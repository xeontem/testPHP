...
function generator(adj, noun) {
  ...
  var randomAdjective = Math.floor(Math.random() * adjectives.length);// No need to string value here
  var randomNoun = Math.floor(Math.random() * nouns.length);// No need to string value here
...
function randomName() {
  var randomNumberAdj = Math.floor(Math.random() * adjectives.length);// No need to string value here
  var randomNumberNoun = Math.floor(Math.random() * nouns.length);// No need to string value here
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  for (var i = 0; i < 32; i++) { // because I see only 32 pizzas on screen
